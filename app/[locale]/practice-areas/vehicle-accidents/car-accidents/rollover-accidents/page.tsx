...
import RolloverAccidentContent from "./RolloverAccidentContent";
import JsonLd from "@/components/JsonLd";
import { getTranslations } from "next-intl/server";

const baseUrl = "https://castillolawsb.com";
const path = "/practice-areas/personal-injury/car-accidents/rollover-accidents";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "PracticeAreasPage.subPages.rolloverAccidents.metadata" });
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const subT = await getTranslations({ locale, namespace: "PracticeAreasPage.subPages.rolloverAccidents" });
