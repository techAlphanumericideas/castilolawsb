...
import ForkLiftContent from "./ForkLift";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "PracticeAreasPage.subPages.forkliftAccidents.metadata" });
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const subT = await getTranslations({ locale, namespace: "PracticeAreasPage.subPages.forkliftAccidents" });
