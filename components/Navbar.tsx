...
import { Menu, X, ChevronDown, Phone, ChevronRight } from "lucide-react"; // Added ChevronRight
...
const Navbar = () => {
  ...
  const [mobileSubExpanded, setMobileSubExpanded] = useState<string | null>(null); // State for mobile sub-menus
  const menuRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (isMenuOpen) {
      document.body.style.overflow = "hidden";
      gsap.to(menuRef.current, { x: "0%", opacity: 1, visibility: "visible", duration: 0.5, ease: "power3.out" });
    } else {
      document.body.style.overflow = "auto";
      gsap.to(menuRef.current, { x: "100%", opacity: 0, duration: 0.5, ease: "power3.in", onComplete: () => { gsap.set(menuRef.current, { visibility: "hidden" }); } });
    }
  }, [isMenuOpen]);
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 w-full bg-white border-b border-gray-100 px-4 md:px-6 py-4 flex items-center shadow-sm transition-all duration-300 z-50`}>
        <div className="w-full flex items-center justify-between gap-2">
          ...
          <div className="flex-none flex items-center border-r border-gray-100 pr-4">
            <Link href="/" className="flex transition-all hover:brightness-110 active:scale-95">
              <Image src="/assets/logo.png" alt={t("logoAlt")} width={220} height={55} className="w-32 md:w-40 h-auto object-contain antialiased" priority unoptimized />
            </Link>
          </div>
            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                >
                  ...
                  {link.dropdownItems && link.nameKey !== "attorneys" && <ChevronDown className={`w-3 h-3 transition-transform duration-300 flex-shrink-0 ${activeDropdown === link.nameKey ? "rotate-180 text-[#C5A059]" : ""}`} />}
                </Link>
                      ...
                      {link.dropdownItems.map((item) => (
                        <div key={item.nameKey} className="relative group/sub">
                          ...
                          >
                            ...
                            {item.subItems && <ChevronRight className="w-3 h-3" />}
                          </Link>

                          {/* SUB-MENU (Option on hover of Defective Products) */}
                          {item.subItems && (
                            <div className="absolute left-full top-0 ml-0.5 w-64 opacity-0 invisible transition-all duration-300 transform translate-x-2 group-hover/sub:opacity-100 group-hover/sub:visible group-hover/sub:translate-x-0">
          ...
          <div className="hidden lg:flex flex-none items-center gap-1 pl-3 border-l border-gray-100">
            <LanguageSwitcher />
            <a href="tel:8052837656" className="group relative flex items-center gap-1.5 px-3 py-2 bg-[#0A1128] text-white rounded-full transition-all duration-300 hover:bg-[#C5A059] hover:shadow-xl hover:-translate-y-0.5">
              <Phone className="w-4 h-4 text-[#C5A059] group-hover:text-white transition-colors flex-shrink-0" />
              <span className="hidden 2xl:inline text-[10px] font-black tracking-widest uppercase whitespace-nowrap">805-283-7656</span>
            </a>
          </div>
          ...
          <div className="lg:hidden ml-auto flex items-center gap-2">
            <LanguageSwitcher />
            <button className="flex items-center z-[110]" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              {isMenuOpen ? <X className="w-7 h-7 text-[#0A1128]" /> : <Menu className="w-7 h-7 text-[#0A1128]" />}
            </button>
          </div>
        </div>
      </nav>
      ...
      {/* Mobile Menu */}
      <div ref={menuRef} className="fixed inset-0 z-[100] bg-white lg:hidden h-screen w-screen overflow-y-auto translate-x-full opacity-0 invisible">
                ...
                {link.dropdownItems ? (
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <Link href={link.href} onClick={() => setIsMenuOpen(false)} className="flex items-center gap-4 text-2xl font-serif font-bold text-[#0A1128]">
                        <span className="text-[#C5A059] text-[10px] font-sans tracking-[0.3em] uppercase opacity-70">0{idx + 1}</span>
                        {t(link.nameKey as any)}
                      </Link>
                      <button onClick={() => setMobileExpanded(mobileExpanded === link.nameKey ? null : link.nameKey)} className="p-2 bg-gray-50 rounded-lg">
                        <ChevronDown className={`w-5 h-5 text-[#C5A059] transition-transform duration-300 ${mobileExpanded === link.nameKey ? "rotate-180" : ""}`} />
                      </button>
                    </div>

                    <div className={`space-y-2 pl-8 overflow-hidden transition-all duration-500 ${mobileExpanded === link.nameKey ? "max-h-[1000px] opacity-100 py-2" : "max-h-0 opacity-0"}`}>
                      {link.dropdownItems.map((subItem) => (
                        <div key={subItem.nameKey} className="space-y-2">
                          <div className="flex items-center justify-between">
                            <Link href={subItem.href} onClick={() => setIsMenuOpen(false)} className="block text-base text-gray-600 hover:text-[#C5A059] py-1">
                              {t(subItem.nameKey as any)}
                            </Link>
                            {subItem.subItems && (
                              <button onClick={() => setMobileSubExpanded(mobileSubExpanded === subItem.nameKey ? null : subItem.nameKey)} className="p-1">
                                <ChevronDown className={`w-4 h-4 text-[#C5A059] transition-transform ${mobileSubExpanded === subItem.nameKey ? "rotate-180" : ""}`} />
                              </button>
                            )}
                          </div>
                          {/* MOBILE NESTED MENU */}
                          {subItem.subItems && (
                             <div className={`pl-4 border-l-2 border-gray-100 overflow-hidden transition-all duration-300 ${mobileSubExpanded === subItem.nameKey ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}>
                                {subItem.subItems.map(sub => (
                                   <Link key={sub.nameKey} href={sub.href} onClick={() => setIsMenuOpen(false)} className="block text-sm text-gray-500 py-2">
                                     {t(sub.nameKey as any)}
                                   </Link>
                                ))}
                             </div>
                  ...
                  <Link href={link.href} onClick={() => setIsMenuOpen(false)} className="flex items-center gap-4 text-2xl font-serif font-bold text-[#0A1128] hover:text-[#C5A059]">
                    <span className="text-[#C5A059] text-[10px] font-sans tracking-[0.3em] uppercase opacity-70">0{idx + 1}</span>
                    {t(link.nameKey as any)}
                  </Link>
                )}
